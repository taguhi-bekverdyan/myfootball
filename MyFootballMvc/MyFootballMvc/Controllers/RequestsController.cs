...
namespace MyFootballMvc.Controllers
{
    public class RequestsController : Controller
    {
        ...
        {
            RequestsIndexViewModel viewModel = await GetRequestsIndexViewModel();
            viewModel.ActiveMenuItem = "invite";
        ...
        public async Task<IActionResult> Invite([FromBody]InviteActionParam param)
        {
            try
            {
                ...
                string invitedUserId = await GetInvitedUserId(param.Id, param.RequestTo, token);

                Team team = await _teamsService.FindTeamByUserId(token, id);

                User invitedUser = await _usersService.FindUserById(token, invitedUserId);
                ...
                await _emailService.Insert(token,email);

                Request request = new Request();
                request.RequestTo = param.RequestTo;
                request.UserId = invitedUserId;
